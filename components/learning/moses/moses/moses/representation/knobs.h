...
protected:
    ...
    // Maps a logical index to a physical index, skipping over disallowed values.
    // The logical index represents the nth allowed value, while the physical
    // index is the actual value in the range [0, Multiplicity).
    int map_idx(int idx) const
    {
        if (idx == _default)
            idx = 0;  // interpreted as "absent", as used below.
        else if (idx == 0)
            idx = _default;
        
        // Count how many disallowed indices come before idx
        // to adjust the physical index accordingly
        int disallowed_before = 0;
        for (int i = 0; i < idx && i < Multiplicity; ++i) {
            if (_disallowed[i]) {
                disallowed_before++;
            }
        }
        return idx + disallowed_before;
    }
};
